...
# addRing / addStripe - uncomment and tune if PDN step is required:
...
# ---- 8. Sign-off checks -----------------------------------------------------------
verify_drc           > $REPORT_DIR/drc_90nm.rpt
verifyConnectivity    > $REPORT_DIR/lvs_conn_90nm.rpt

# ---- 9. Final timing & power (post-route) ---------------------------------------
timeDesign -postRoute > $REPORT_DIR/timing_postroute_90nm.rpt
report_power          > $REPORT_DIR/power_postroute_90nm.rpt
report_area           > $REPORT_DIR/area_postroute_90nm.rpt

# ---- 10. Save design + GDS streamout ---------------------------------------------
saveDesign $OUT_DIR/db/agrisense_ipms_top_90nm.enc

if {[file exists $GDS_LAYERMAP]} {
    streamOut $OUT_DIR/gds/agrisense_ipms_top_90nm.gds \
...
puts "============================================================"
puts "AgriSense-IPMS 90nm P&R complete."
puts "Reports : $REPORT_DIR"
puts "DB      : $OUT_DIR/db/agrisense_ipms_top_90nm.enc"
puts "GDS     : $OUT_DIR/gds/agrisense_ipms_top_90nm.gds (if layermap found)"
puts "============================================================"
